...
import { bnToFixed } from '~~/helpers/utils';
    ...
    <>
      <List.Item
        ...
        <List.Item.Meta
          avatar={<></>}
          title={pool.name}
          description={`${pool.name} pool with total supply of ${bnToFixed(pool.totalSupply, 4)}`}
        />
        <Row gutter={20} style={{ display: 'flex', justifyContent: 'center' }}>
          ...
          <Col span={4}>
            <Statistic
              title="Amount staked"
              value={`${bnToFixed(pool.userInfo.balance, 4)} ${pool.symbol}`}
              precision={2}
            />
          </Col>
          <Col span={4}>
            <Statistic title="Total Supply" value={bnToFixed(pool.totalSupply, 4)} />
          </Col>
          <Col span={4}>
            <Statistic title={`Your ${pool.symbol} balance`} value={bnToFixed(balance, 4)} />
          </Col>
        </Row>
      </List.Item>
    </>
